...
\section{Logit-Based Score Extraction}\label{app:logit_score_extraction}

This appendix provides the technical details for the logit-based scoring procedure described in Section~\ref{sec:methods_llm_procedure}. The procedure converts each prompt-document pair into a continuous Expected Value (EV) score and a normalized entropy diagnostic by restricting the model's next-token distribution to the valid rating tokens.
