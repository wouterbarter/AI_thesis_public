...
% TODO Maybe change semicolon in last sentence
...
% TODO Explain that formative representations decompose the target construct into constituent dimensions
...
\end{description}
% TODO The relation to the RQs is a bit unnatural, maybe weave them in 
...
% TODO also why is J_m suddenly criteria? What is C_m?
...
Predictive-validity evidence is evaluated in two ways. First, we compare in-sample model fit against the structured baseline using log-likelihood, McFadden pseudo-$R^2$, AIC, and BIC. Improvements are reported relative to the baseline: $\Delta$LL is defined as $LL_{\mathrm{model}} - LL_{\mathrm{baseline}}$, while $\Delta$AIC and $\Delta$BIC are defined as $AIC_{\mathrm{baseline}} - AIC_{\mathrm{model}}$ and $BIC_{\mathrm{baseline}} - BIC_{\mathrm{model}}$, respectively. All three quantities are oriented so that positive values indicate improvement over the structured baseline. %TODO structured or structural?
...
%TODO maybe also include how values are interpreted? What is low/high VIF, Cohen's d, etc.
