...
This section describes the experimental procedure used to obtain the LLM evaluations and compare different forms of construct specification. Through experiments, we aim to answer the research questions proposed at the beginning: 
...
All evaluations use a standardized prompt structure. For each artefact $d_i$ and criterion $c_j \in C$, the model input is defined as the triplet
\[
    \mathbf{u}_{ij} = [\, s \,;\, c_j \,;\, d_i \,].
\]
where $s$ denotes the system context and task instruction, $c_j$ denotes the criterion specification, and $d_i$ denotes the target artefact $i$. The three components are defined as follows:
...
\begin{itemize} 
    ...
    \item $c_j$ \textbf{(Criterion Specification):} The prompt-level measurement target. This includes the semantic definition of the construct or criterion being evaluated, together with the Likert-scale anchors used to define the rating levels. 
    \item $d_i$ \textbf{(Target Document):} The artefact to be evaluated, such as a Q\&A pair in FeedbackQA or the relevant Deal text in Barter Deals. 
\end{itemize}

Across experimental conditions, the system context $s$ and target document $d$ are held constant. The experimental manipulation is implemented through the content and structure of the criterion specification $c_j$. The experimental design differs per dataset; for \textit{FeedbackQA}, see Section~\ref{sec:qa_experimentaldesign}, for \textit{Barter Deals}, see Section~\ref{sec:barter_experimentaldesign}. 


\subsubsection{Logit-Based Score Extraction}\label{sec:meth_decoding}

After constructing the evaluation triplet $\mathbf{u}_{ij}$, we instantiate and supply it to the LLM in Chat Template format, consisting of successive turns with fixed roles. The \emph{system} turn contains $s$ and $c_j$\footnote{For the informed baseline condition, $c_j$ represents a composite set of criteria rather than an isolated dimension; see Section~\ref{sec:qa_experimentaldesign}.}, and the \emph{user} turn contains $d_i$.

Generation begins at the \emph{assistant} turn, which we manually prefix with the literal string \texttt{Rating: } to condition the next-token distribution toward the evaluation tokens (see Appendix~\ref{app:apply_chat_template} for exact templates). We then perform a single forward pass to obtain the next-token logits $\mathbf{z}(\mathbf{u}_{ij}) \in \mathbb{R}^{|\mathcal{V}|}$ for vocabulary $|\mathcal{V}|$. 

We restrict the logits to the subset of rating tokens $\mathcal{S} = \{ '1', \dots, 'L' \}$, where $L$ represents the maximum integer of the specified Likert scale. This yields the restricted logit vector $\mathbf{z}_{\mathcal{S}}(\mathbf{u}_{ij}) = \{ z_l(\mathbf{u}_{ij}) \}_{l \in \mathcal{S}}$.

The probability distribution over the rating tokens is obtained by applying a softmax to the restricted logit vector:
\begin{equation}
P_{\mathcal{S}}(l \mid \mathbf{u}_{ij}) = \frac{\exp(z_l(\mathbf{u}_{ij}))}{\sum_{h \in \mathcal{S}} \exp(z_h(\mathbf{u}_{ij}))}.
\end{equation}
...
To make entropy comparable across scales with different numbers of categories, such as the four-point scale used for FeedbackQA and the seven-point scale used for Barter Deals, we apply a normalization factor. Let $K = |\mathcal{S}|$ denote the number of valid rating categories. The normalized entropy $H_{ij}$ for prompt sequence $\mathbf{u}_{ij}$ is defined as:
\begin{equation}\label{eq:normalized_entropy}
H_{ij} = - \frac{1}{\ln(L)} \sum_{l \in \mathcal{S}} P_{\mathcal{S}}(l \mid \mathbf{u}_{ij}) \ln P_{\mathcal{S}}(l \mid \mathbf{u}_{ij}).
\end{equation}
